...
**本文讲**：在 LAMMPS 里怎么用 `aveforce` / `setforce` / `nve` 把一块墙做成活塞、给流体单独控温、并输出局部应力与密度剖面来验证控压是否奏效。**不讲**：怎么把这个「板—溶液—板」受限体系搭出来（见 [受限溶液建模](C01-受限溶液建模.md)）；压力张量与局部应力的理论（见 [轨迹分析与宏观性质](../00-知识文档/K16-轨迹分析与宏观性质.md)）。

**前置**：手上已有一个「板—溶液—板」的 `data.lmp`（可直接用 [受限溶液建模](C01-受限溶液建模.md) 产出的约 2000 SPC/E 水 + 上下墙体系）；跑过基本的 LAMMPS 输入脚本。
...
直接读入 [受限溶液建模](C01-受限溶液建模.md) 生成的 `data.lmp`（本例约 2000 SPC/E 水 + 上下同质碳墙）。原子类型以你的 `.ff` 为准；同质墙常见约定：

| type | 元素 | 角色 |
|------|------|------|
| 1 | 墙原子（如 `C` / `Cw`） | **上墙 → 活塞** |
| 1（同 type）或后续墙 type | 墙原子 | **下墙 → 底座**（固定；与上墙共用同一 `wall` 时 type 常相同，靠坐标分组） |
| 其余 | `H` / `O`（SPC/E） | **流体** |

若你用的是异质上下墙，按下墙元素拆成多个 type 即可；下面脚本里的 `group` 按**角色**划分，不要死记 type 编号。

按角色分组。同质墙上下 type 相同时，用坐标区分活塞与底座：

```
group wall   type 1
group fluid  type 2 3      # 视 data.lmp 中水的 type 而定
region  upper_reg block INF INF INF INF 0 INF units box
region  lower_reg block INF INF INF INF INF 0 units box
group   upper region upper_reg
group   lower region lower_reg
group   upper intersect upper wall   # 上半盒 ∩ 墙 → 活塞
group   lower intersect lower wall   # 下半盒 ∩ 墙 → 底座
```

若上下墙 type 本就不同，可直接 `group upper type 1`、`group lower type 2 …`，不必用 region。
