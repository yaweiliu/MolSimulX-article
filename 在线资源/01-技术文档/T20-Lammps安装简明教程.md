...
| 路线                              | 适合谁                                                    | 优点                         | 注意                                        |
| ------------------------------- | ------------------------------------------------------ | -------------------------- | ----------------------------------------- |
| **Mac：Homebrew**                | 已按 [Mac与Ubuntu开发环境配置](T19-Mac与Ubuntu开发环境配置.md) 装好 brew | 一行安装；自带 serial / MPI 可执行文件 | **并行 PPPM 易踩 FFTW 坑**（见第三节）；要稳请自编译 KISS |
| **Ubuntu / Debian / WSL：`apt`** | 系统较新                                                   | 简单；随系统更新                   | 发行版仓库版本可能略落后于官网最新 stable                  |
| **源码 CMake 自编译**                | **Mac 推荐**（尤其要用 `kspace_style pppm`）；要开特定 package / GPU；或手里有改写源码 | 版本与 FFT 可控；可避开 brew FFTW   | 要准备编译器 + CMake；别和 brew/apt 的 `lmp` 抢 PATH |
...
1. **Mac：** `brew install lammps` 可冒烟；**MPI + PPPM 请自编译 `-D FFT=KISS`**（第三节、第五节），与 brew 版分目录并存。  
2. **Ubuntu / WSL：** `sudo apt install lammps` → 常用 `lmp`。  
3. **改写 / 新增源码 / 常用包：** CMake 编到 `$HOME/…`；Mac 推荐 `most` + `nolib` + `FFT=KISS` + MPI；也可用 `-D Lammps_MACHINE=…` 生成 `lmp_名字`。  
