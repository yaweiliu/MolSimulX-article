...
| 步              | 输入                                                  | 输出                                   | 输出用途                                                                                       |
| -------------- | --------------------------------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------ |
| **生成 Packmol 输入** | 一种或多种分子的坐标文件（常见为 `_ff.xyz`；XYZ 物种另可填 `atoms_per_mol`）和对应 `.ff`，以及分子个数、盒子尺寸等设置 | `pack.inp`、各物种 `*_pack.xyz` | `pack.inp` 是可继续编辑的装盒配置；`*_pack.xyz` 是提供给 Packmol 的单分子坐标 |
