...
![](../../images/articles/在线工具/M10-MDStudio结构变换/web/M10-hero-strcutre_trans.webp)
...
输出第二行保留原有前缀（分子名、`.ff` 引用等），并按需重写 `cell`。本 Tab 仅限 **VIP**；任务在后台异步执行，可随时终止。
...
- **输出**：`{stem}_trans.xyz`，写入**同一文件夹**，仍指向同一个 `.ff`；再次对同一 stem 运行会**覆盖同名输出**，不会自动编号。
...
- **输入原子数**：提交时检查所选结构，必须不超过 **1000 原子**。
- **扩胞 / 建胞后的规模**：本 Tab 不检查 `max_box_atoms`，也不检查 `.ff` 的键 / 角 / 二面角 / 离面角计数；变换后的原子数可增长，后续装盒或测试再按各自闸门处理。
- **墙钟与队列**：`msxtrans` 最多运行 **60 秒**；全站同时运行最多 **1** 个任务、每用户运行加排队合计最多 **1** 个、全站排队最多 **15** 个。
- **写入闸门**：输出仍受每目录 **50 项**与 VIP 工作区 **200 MB** 配额约束。
...
5. VIP 输入必须 ≤1000 原子；本 Tab 不查 box / `.ff` 拓扑上限，但输出仍受工作区写入闸门。
6. `_trans.xyz` 同名覆盖；任务墙钟 60 秒，队列并发为 1 / 1 / 15。
