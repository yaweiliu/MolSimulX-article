...
| 约束 | 当前值 | 说明 |
|------|--------|------|
| **原子数上限** | **5000** | 超过则 ① / ② 直接拦截；免费档通常先撞上装盒总原子 ≤1000，请回搭建盒子减少分子数重装 |
| **单次墙钟上限** | **300** 秒（`lammps_test`） | 到时杀进程树；可能留下半成品日志 / 轨迹，属超时而非成功完成 |

① **生成测试脚本**为同步；② **运行测试模拟**入队（当前全站同时运行 3、每用户合计 1、排队上限 20）。权威额度见 [MDStudio使用须知与限制](M02-MDStudio使用须知与限制.md)。需要更大体系或更长模拟，请下载输入到本地 / 集群运行。
