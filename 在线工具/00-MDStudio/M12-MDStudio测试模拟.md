...
- 任务**正常结束**为通过；
- 若撞上墙钟上限，进程会被终止，可能留下**不完整**日志 / 轨迹供诊断——这是**超时中止**，不是「成功跑完」；
- 日志无致命错误，热力学量有限、无发散；
- 输入文件仍可在资源管理器下载。
...
测试模拟有两条硬约束（当前机房配置；界面也会显示生效值）：

| 约束 | 当前值 | 说明 |
|------|--------|------|
| **原子数上限** | **1000** | 超过则 ① / ② 直接拦截，请回搭建盒子减少分子数重装 |
| **单次墙钟上限** | **300** 秒（`lammps_test`） | 到时杀进程树；可能留下半成品日志 / 轨迹，属超时而非成功完成 |

① **生成测试脚本**为同步；② **运行测试模拟**入队（当前全站同时运行 1、每用户合计 1、排队上限 15）。权威额度见 [MDStudio使用须知与限制](M02-MDStudio使用须知与限制.md)。需要更大体系或更长模拟，请下载输入到本地 / 集群运行。
...
| 跑到一半就停了 | 多半撞墙钟（当前 300 s）：属超时中止，看日志是否已有足够信息；生产请下载离线跑 |
...
测试模拟刻意做小、做短、去掉长程静电求解，就是为了在有限墙钟内给出「能不能跑」的结论。真正的采样需要长程静电、更长时间、合适系综与平衡判据，应使用完整的 `in.lmp`，下载到本地或集群运行（见 [本地与集群文件传输](T09-本地与集群文件传输.md)、[集群与 SLURM 简明教程](T10-集群与SLURM简明教程.md)）。
...
5. 原子数上限 **1000**、墙钟 **300** 秒；超时是中止不是成功。生产采样请下载后离线跑。
