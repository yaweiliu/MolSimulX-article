...
本文详细介绍六大类模板及各自参数、真空层与 CIF 导出的处理、命名规则、原子数校验与权限，以及生成后如何接力场（框架材料常用 EQeq）与结构变换。这里只产出**结构**，不产出力场。

![](../../images/articles/在线工具/M07-MDStudio周期分子/web/M07-hero-periodic_molecule.webp)
...
选择类别 + 参数
   ...
   原子模型（含/不含晶胞）
        │  补足三维晶胞（有限体系加真空盒）
        ▼
   {name}.cif  →  力场生成 / 结构变换 / 装溶剂
```

本 Tab 仅限 **VIP**，生成是**同步**的，不进入后台队列，也没有任务墙钟。底部三个按钮分工如下：

| 按钮 | 作用 |
|------|------|
| **预览结构** | 按当前参数生成隐藏的 `preview.crystal.cif` 并在右侧查看；该文件不显示在资源管理器中，重复预览会覆盖它 |
...
> **复杂周期结构暂需本地自备。** 本 Tab 覆盖的是上面六类相对规整的晶体 / 表面 / 纳米结构；像 **COF、MOF、氧化石墨烯（GO）** 这类拓扑复杂、需要专门构建器或数据库的周期材料，目前**请在本地用相应工具生成 `.cif`，再上传到工作区**接后续的力场生成 / 结构变换 / 装溶剂。相关的构建工具与模块**后续会陆续接入** MDStudio 辅助生成，敬请期待。
...
字段缺省时，MX2 使用 `a = 3.18 Å`、`thickness = 3.19 Å`，重复数为 `2 / 2 / 1`。
...
| 产物 | 用途 |
|------|------|
| `{name}.cif` | 力场生成、结构变换、装溶剂 / 客体、可视化 |

分子名默认 `crystal`，写入资源管理器**当前文件夹**；同名自动去重（`crystal`、`crystal2`…）。分子名不能含路径分隔符或以 `.` 开头。

「预览结构」只覆盖隐藏的 `preview.crystal.cif`，不会在资源管理器里留下正式条目；只有「仅保存 cif」或「用于力场生成」才会按上面的命名规则落盘。

---

## 六、限制

- **原子数两次校验**：生成前先按参数**预估**原子数，生成后再按**实际**原子数复核，两次都必须不超过 VIP 有效单分子上限 **1000 原子**。预览与正式保存均受此限制。
- **条目上限**：正式保存前检查当前目录最多 **50 项**；预览文件通常覆盖隐藏的同名文件，不新增可见条目，但首次创建隐藏预览时仍可能触达配额。
...
- **一键接力场**：点「用于力场生成」会写出 CIF 并跳到 [MDStudio力场生成](M09-MDStudio力场生成.md) 载入它。
  - **框架 / 周期材料常用 EQeq 电荷**：EQeq 需要 CIF 中存在可实际解析的晶胞参数；只有文件名或周期材料标签不够。无实际 cell 的有限团簇 CIF 会被拒绝。
- **扩胞 / 变换**：需要更大或近正交超胞、或把分子堆成晶体时接 [MDStudio结构变换](M10-MDStudio结构变换.md)。
- **装溶剂 / 客体**：把框架与溶剂一起送入 [MDStudio搭建盒子](M11-MDStudio搭建盒子.md)。
...
2. 可先「预览结构」确认形貌；预览写入隐藏的 `preview.crystal.cif`，正式按钮才生成可见 CIF。
3. 有限体系导出 CIF 会补真空盒；需要真隔离请显式填 `vacuum`。
4. 生成前预估、生成后实测都必须 ≤1000 原子（预览同样受限）。
5. 框架材料常配 EQeq 电荷，且 EQeq 必须读到 CIF 中的实际晶胞。
6. 保存后可一键接力场生成，或先做结构变换。
