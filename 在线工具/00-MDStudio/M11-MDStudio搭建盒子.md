...
1. **① 生成 Packmol**：读取已选物种坐标，按盒子尺寸写出 `pack.inp` 和各组分的 `*_pack.xyz`；本阶段不建拓扑。
...
物种选择阶段只要求结构可定位配套 `.ff`，**不检查 `max_molecule_atoms`**。
...
- **已选（右）**：勾选后进入右栏，按行显示；每行可**上移 / 下移**调整顺序，并填写两个数：
  - **个数**（最小 1，默认 1）：Packmol 复制**整份物种文件**的次数；
  - **atoms_per_mol**（仅 `.xyz` 物种显示，默认 **0**）：写 `data.lmp` 时是否把整块坐标拆成多个 molecule。`0` = 不拆（与旧行为一致）；`>0` = 按每分子原子数切开。
- **清空并重置参数**：清空已选并把整表参数复位为默认。

多组分示例可直觉对应配比：溶质 N 个 + 水 M 个；离子液体阴阳离子按化学计量比；混合溶剂按摩尔比给个数。

### 3.1 `atoms_per_mol`：分子晶体 / 多分子坐标块怎么拆

装盒默认把「一个物种文件」当成**一个分子模板**。若你先用「结构变换 · 分子晶体」堆出多根棒 / 多个分子，再把整块 `_trans.xyz` 作为物种、`份数=1` 丢进 Packmol（常配合 `fixed`），写出的 `data.lmp` 里整块会变成 **1 个 molecule**——对 `fix rigid molecule` 等后续设置是错的，对后续分子各个分子的性质也不友好。

此时在该物种行把 **atoms_per_mol** 设为**单分子原子数**（例如一根 11 珠棒填 `11`）：

| 量 | 含义 |
|----|------|
| 物种文件原子数 | Packmol 放进去的整块大小（如 100×11） |
| **个数** | 整块复制几次（晶体通常 `1`） |
| **atoms_per_mol** | 写 data 时每个 LAMMPS mol 的原子数；`0` 不拆 |
| 写出的 mol 数 | `个数 × 文件原子数 / atoms_per_mol` |

**前提（务必满足）：**

1. 文件内原子顺序为「分子 1 的全部原子 → 分子 2 → …」**连续分块**（结构变换堆叠的产物通常如此）；
2. 每一块与第一块**同构**（同原子数、同拓扑）；
3. 键不跨块。顺序错乱会导致 type / 键套错、刚体分组错误。

校验：文件原子数必须能被 `atoms_per_mol` 整除。切分**只影响 ③ 生成 Lammps**，不影响 ①② 的 Packmol 原子计数。右栏标题旁的 **?** 有同样说明。

> 溶剂等普通单分子物种保持 `atoms_per_mol = 0` 即可；可与「晶体块 + atoms_per_mol」同盒共存。

物种顺序与各物种个数不仅写进 `pack.inp`，也决定了 `simbox.xyz` 中原子的排列次序，进而决定 ③ 生成 Lammps 时坐标与拓扑的对应关系（见第五节 ③ 的关键提醒）。**跑完 Packmol 后不要再改顺序、个数或 atoms_per_mol**，否则要从 ① 重新生成并重跑 ②。

> 物种选择不套用单分子原子数上限；真正的总体系规模在 ②、③ 分阶段检查。此处「单分子原子数」指物种**文件**原子数，与是否拆 mol 无关。
...
本阶段不检查盒子总原子数，也不建立拓扑或检查 `max_ff_*`；后台 kind 为 `msxbox_packmol`，墙钟 **60 秒**。
...
运行前会做规模校验：程序**同时**按表单估算和按 `pack.inp` 里 `structure × number` 估算总原子数，取较大值比对有效盒子上限：免费 **100 原子**、VIP **100000 原子**。免费超限硬拦；VIP 大任务可挂起并生成 `run_packmol.sh`。本阶段 `packmol` 墙钟 **300 秒**。

### ③ 生成 Lammps

点「③ 生成 Lammps」，平台先校验 `simbox.xyz` 的原子数是否与当前物种×个数一致，一致后写出 `data.lmp`、`in.lmp`（必要时 `pair.lmp`）。

本阶段硬顶为 **2×有效盒子上限**，即免费 **200 原子**、VIP **200000 原子**；建立键合拓扑时还检查 `.ff` 的键 / 角 / 二面角 / 离面角上限 **1000 / 1000 / 5000 / 1000**。`msxbox_lammps` 墙钟 **60 秒**。
...
平台按「每个物种个数 × 物种文件原子数」求和得到体系总原子数。选择物种与 ① 不套用单分子 / 盒子原子上限；②、③ 再分阶段校验：

超限时的行为分几种：

| 情况 | 行为 |
|------|------|
| **① 生成 Packmol** | 不查总原子数、不建拓扑；墙钟 60 秒 |
| **② 超出有效盒子上限** | 免费超过 100 原子硬拦；VIP 超过 100000 原子可挂起并生成 `run_packmol.sh`；已有 `simbox.xyz` 超过 2×有效上限仍硬拦；墙钟 300 秒 |
| **③ 生成 Lammps** | 硬顶为 2×有效盒子上限，并检查有键体系的 `.ff` 拓扑计数；墙钟 60 秒 |
| **已有合规 `simbox.xyz`** | 模式 B 可跳过 ①②，直接点 ③；表单物种顺序与个数必须和该文件一致 |

三种后台任务共用队列：全站同时运行最多 **1** 个、每用户运行加排队合计最多 **1** 个、全站排队最多 **15** 个。所有产物还受每目录 **50 项**及工作区总量（免费 **50 MB**、VIP **200 MB**）约束。
...
2. 打开搭建盒子，勾选物种、填个数（分子晶体可另填 **atoms_per_mol**）、调整顺序。
...
| 分子晶体整块变成 1 个 molecule | 在该物种填 **atoms_per_mol** = 单分子原子数；并确认文件内原子按分子连续分块 |
| atoms_per_mol 报不能整除 | 文件原子数 ÷ atoms_per_mol 须为整数；检查堆叠是否完整、是否选错文件 |
...
2. 物种必须带 `.ff`，但选择时不检查单分子原子数；`atoms_per_mol` 仅适用于 XYZ。
3. 多分子**不合并** LAMMPS type，便于人工调物种间相互作用，也避免异源同名 type 混参。
4. **③ 的物种顺序与个数必须与生成 `simbox.xyz` 时一致**；校验只看总原子数，改动后务必从 ① 重来。
5. ① 不查盒子原子数；② 按免费 100 / VIP 100000 检查；③ 硬顶为其 2 倍并检查 `.ff` 拓扑。
6. 三阶段墙钟为 60 / 300 / 60 秒，队列并发为 1 / 1 / 15。
