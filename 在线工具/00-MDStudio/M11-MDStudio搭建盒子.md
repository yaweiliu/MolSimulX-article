...
运行前会做规模校验：程序**同时**按表单估算和按 `pack.inp` 里 `structure × number` 估算总原子数，取较大值比对有效盒子上限：免费 **1000 原子**、VIP **100000 原子**。免费超限硬拦；VIP 大任务可挂起并生成 `run_packmol.sh`。本阶段 `packmol` 墙钟 **300 秒**。

### ③ 生成 Lammps

点「③ 生成 Lammps」，平台先校验 `simbox.xyz` 的原子数是否与当前物种×个数一致，一致后写出 `data.lmp`、`in.lmp`（必要时 `pair.lmp`）。

本阶段硬顶为 **2×有效盒子上限**，即免费 **2000 原子**、VIP **200000 原子**；建立键合拓扑时还检查 `.ff` 的键 / 角 / 二面角 / 离面角上限 **6000 / 30000 / 50000 / 20000**。`msxbox_lammps` 墙钟 **60 秒**。
...
| **② 超出有效盒子上限** | 免费超过 1000 原子硬拦；VIP 超过 100000 原子可挂起并生成 `run_packmol.sh`；已有 `simbox.xyz` 超过 2×有效上限仍硬拦；墙钟 300 秒 |
| **③ 生成 Lammps** | 硬顶为 2×有效盒子上限，并检查有键体系的 `.ff` 拓扑计数；墙钟 60 秒 |
| **已有合规 `simbox.xyz`** | 模式 B 可跳过 ①②，直接点 ③；表单物种顺序与个数必须和该文件一致 |

三种后台任务共用队列：全站同时运行最多 **3** 个、每用户运行加排队合计最多 **1** 个、全站排队最多 **20** 个。所有产物还受每目录 **50 项**及工作区总量（免费 **50 MB**、VIP **200 MB**）约束。
