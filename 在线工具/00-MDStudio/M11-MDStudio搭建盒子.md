...
- **已选（右）**：勾选后进入右栏，按行显示；每行可**上移 / 下移**调整顺序，并填写两个数：
  ...
  - **atoms_per_mol**（默认 **0**）：写 `data.lmp` 时是否把整块坐标拆成多个 molecule。`0` = 不拆（与旧行为一致）；`>0` = 按每分子原子数切开。
...
装盒默认把「一个物种文件」当成**一个分子模板**。若你先用「结构变换 · 分子晶体」堆出多根棒 / 多个分子，再把整块 `_trans.xyz` 作为物种、`个数=1` 丢进 Packmol（常配合 `fixed`），写出的 `data.lmp` 里整块会变成 **1 个 molecule**——对 `fix rigid molecule` 等后续设置是错的。
...
> 每个物种自身的原子数、以及「个数 × 单分子原子数」求和得到的体系总原子数都有上限；超限的处理见第十节。（此处「单分子原子数」指物种**文件**原子数，与是否拆 mol 无关。）
