...
不想画分子、也不想到处找水模型文件——**分子仓库**当前收录约 **580** 条可直接导入的结构（离子约 480、离子液体 54、水模型 13、小分子 / 气体约 30），每条都带配套力场。可先筛选浏览、3D 预览并查看来源，再一键导入工作区。

**分类 / 标签 / 搜索 → 预览（结构 + 来源）→ 导入（结构 + `.ff`）→ 搭建盒子。**

条目**已带力场**，导入后通常可跳过力场生成，直接进入装盒。
...
关键特点：每个条目是「结构 + 它声明的 `.ff`」成对包。力场已写好，导入后可直接进入 [MDStudio搭建盒子](M11-MDStudio搭建盒子.md)。
...
- **无导入次数限制**：不限每日 / 每次导入条数；仅受工作区总大小配额约束（见 [MDStudio使用须知与限制](M02-MDStudio使用须知与限制.md)）。
