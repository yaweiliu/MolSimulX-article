        ...
        if connect:
            with open(str(output_dir / "result_connect.dat"), "w") as fh:
                for i in range(self.atoms.n_atoms):
                    sel = self.select_atoms(f"index {i}")
                    connect_list = [f"{sel.atoms[0].id}"]
                    for bd in sel.bonds:
                        other = bd.atoms[1] if bd.atoms[0].id == sel.atoms[0].id else bd.atoms[0]
                        connect_list.append(f"{other.id}")
                    fh.write("CONECT " + " ".join(connect_list) + "\n")
